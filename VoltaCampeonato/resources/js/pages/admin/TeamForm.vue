  ...
  <div class="max-w-2xl animate-fade-in">
    ...
    <form @submit.prevent="handleSubmit" class="card-gradient rounded-xl p-6 space-y-5">
      <div><label class="block text-sm font-medium text-surface-300 mb-1.5">Campeonato *</label>
        <select v-model="form.championship_id" required class="w-full px-4 py-2.5 rounded-lg bg-surface-800 border border-surface-700 text-white focus:outline-none focus:ring-2 focus:ring-primary-500/50"><option value="">Seleccionar...</option><option v-for="c in championships" :key="c.id" :value="c.id">{{ c.name }}</option></select></div>
...
const form = reactive({ championship_id: '', name: '', course: '', parallel: '', category: '', color: '#10b981', captain_name: '' });
onMounted(async () => {
  try { const d = await get('/campeonatos'); championships.value = d.data || []; } catch {}
  if (isEdit.value) { try { const d = await get(`/equipos/${route.params.id}`); const t = d.data || d; Object.keys(form).forEach(k => { if (t[k] !== undefined && t[k] !== null) form[k] = t[k]; }); } catch {} }
});
